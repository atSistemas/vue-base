...
function inputOperator(state, action) {
  ...
  const calculated = (newValue) ?
    calculate(prevOperator, prevValue, nextValue) : prevValue;

  return state
    .set('nextValue', 0)
    .set('newValue', true)
    .set('display', calculated)
    .set('prevValue', calculated)
...
function calculate(operator, prevValue, nextValue) {
  ...
  return operator ? calc[operator]() : prevValue;
}
...
function result(state) {
  ...
  const prevValue = state.get('prevValue');
  const nextValue = state.get('nextValue');
  const calculated = calculate(operator, prevValue, nextValue);
